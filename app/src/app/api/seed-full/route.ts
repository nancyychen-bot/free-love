...
import { users, profiles, introductions, conversations, messages, lifeAnswers, lifeSignals, userState } from '@/lib/db/schema';
...
export async function POST(request: Request) {
  try {
    ...
    const existingIntro = await db
      ...
      .where(
        or(
          and(eq(introductions.userAId, targetUser.id), eq(introductions.userBId, maraId)),
          and(eq(introductions.userAId, maraId), eq(introductions.userBId, targetUser.id))
        )
      )
      .limit(1);
    ...
    for (const seed of CONVO_SEEDS) {
      ...
      const existingConvo = await db
        ...
        .where(
          or(
            and(eq(conversations.userAId, targetUser.id), eq(conversations.userBId, seedUser.id)),
            and(eq(conversations.userAId, seedUser.id), eq(conversations.userBId, targetUser.id))
          )
        )
        .limit(1);

      if (existingConvo.length === 0) {
        // Create a dummy introduction for the conversation
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + 5);
        ...
        for (let i = 0; i < seed.messages.length; i++) {
          ...
          await db.insert(messages).values({
            ...
            senderId: msg.from === 'you' ? targetUser.id : seedUser.id,
            body: msg.text,
